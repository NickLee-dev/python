...
w(wt), x(xt) : 새로 쓰기(write text : wt)
    - w(wt) : 대상 파일이 없으면 자동으로 파일을 새로 만들어서 열기함
            대상 파일이 있으면 파일 안의 내용을 모두 지우고 새로 쓰리 상태로 열기함
...
def  test_fwrite():
    ...
    f.write('12345')
    f.write('파일에 저장 확인!') # 텍스트 파일 인코딩이 'ms949'(ISO8859-1) 문자셋임
    f.write('★!@#')
    f.close()
...
def test_fwrite2():
    ...
    f = open("D:\\python_workspace\\day7Project\\fileio\\testb.txt", 'x', encoding='utf-8')
...
def test_osmodule():
    ...
    print(os.getlogin()) # ict01-13
    # 현재 작업중인 디렉토리 조회
    print(os.getcwd()) # D:\python_workspace\day7Project\fileio
    ...
    # os.mkdir(work_dir) # .mkdir() : 디렉토리 만들어라, 주의 : 같은 이름의 디렉토리가 있으면 error
    ...
    f.write('파이썬으로 디렉토리 만들고, 만든 디렉토리로 작업폴더 변경하고 파일 저장 테스트')
    st = '''변경된 디렉토리에 파일을 새로 만들고 
    유니코드로 인코딩 된 글자들을 파일에 기록 저장
    확인용 파일'''
    f.write(st)
    f.close()
...
def test_writelines():
    ...
    # f.write(item) # write() 는 str 만 기록할 수 있음, 리스트 저장 못 함
    lst = [10,20,30] # TypeError: write() argument must be str, not int
    for data in lst:
...
# readline() : 파일 안의 내용을 한 줄씩 읽음, 마지막 라인을 읽고나서 더이상 읽을 라인이 없으면 None(False) 리턴함
# readlines() : 파일의 내용을 줄 단위(아이템)로 모두 읽어서 list로 리턴함 [ '한줄' , '한줄' , ... ]
def test_fread():
    print(os.getcwd())
    ...
    while True:
        line = f.readline()
        if not line: # line 변수의 값이 없다면(None이면)
            break
        print(line,end='')
